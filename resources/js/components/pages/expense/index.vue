        ...
        <div class="content-header">
            <div class="container">
                <div class="d-flex justify-content-between">
                    <h3>Expense</h3>
                    <div class="">
                        <router-link :to="{name: 'expense/create'}" class="btn btn-info">
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <table id="example1" class="table table-bordered table-striped mb-2">
                                    ...
                                    <tbody>
                                        <tr v-show="expenses.length" v-for="(expense, index) in expenses" :key="expense.id">
                                            <td>{{index + 1}}</td>
                                            <td>{{expense.details}}</td>
                                            <td>{{expense.amount}}</td>
                                            <td>
                                                <router-link :to="{name: 'expense/edit', params:{id: expense.id}}" class="btn btn-primary btn-sm">
                                                    <i class="fas fa-edit"></i>
                                                </router-link>
                                                <button class="btn btn-danger btn-sm" @click="destroy(expense.id)">
...
export default {
        ...
        methods: {
            getData() {
                this.$Progress.start()
                axios.get('/api/expense?page='+this.pagination.current_page)
                    .then(response => {
            ...
            searchData() {
                ...
                axios('/api/search/expense/'+this.query+'?page='+this.pagination.current_page)
                    .then(response => {
                        this.expenses  = response.data.data
                        this.pagination = response.data.meta
                        this.$Progress.finish()
